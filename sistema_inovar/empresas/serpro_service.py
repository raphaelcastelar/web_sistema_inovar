...
def consultar_extrato_serpro(cnpj_empresa, periodo_apuracao):
    ...
    Retorna os dados do extrato sem o PDF.
    """
    tokens = get_serpro_token()
    if not tokens:
        return {"sucesso": False, "erro": "Falha na autenticação com a API Serpro."}
    ...
    logger.info(f"Enviando payload para consultar dados de extrato: {json.dumps(payload, indent=2)}")
...
def download_extrato_pdf_serpro(cnpj_empresa, numero_das):
    """
    Chama o serviço CONSEXTRATO16 para obter o PDF do extrato de um DAS específico.
    """
    tokens = get_serpro_token()
    if not tokens:
