...
const pastaConfig = {
    'documentos_constitutivos': { label: 'Doc. Constitutivos', icon: FolderIcon },
    'departamento_pessoal': { label: 'Dpto. Pessoal', icon: FolderIcon },
    'xml': { label: 'XML', icon: FolderIcon },
    'simples_nacional': { label: 'Simples Nacional', icon: FolderIcon },
    'outros': { label: 'Outros', icon: FolderIcon },
};
...
const YearMonthAccordion = ({ files, selectedFiles, toggleFileSelection }) => {
    ...
    if (!files || files.length === 0) return <div className="text-center py-10"><p className="text-gray-500 dark:text-gray-400">Nenhum arquivo nesta pasta.</p></div>;
    if (sortedYears.length === 0) return <div className="text-center py-10"><p className="text-gray-500 dark:text-gray-400">Não foi possível agrupar os arquivos por data.</p></div>;
            ...
            {sortedYears.map(year => (
                <div key={year} className="bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                    <button onClick={() => setActiveYear(activeYear === year ? null : year)} className="w-full flex justify-between items-center p-4">
                        ...
                        <span className={`transform transition-transform duration-200 ${activeYear === year ? 'rotate-180' : ''}`}><svg className="h-5 w-5 text-gray-500 dark:text-gray-400" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd" /></svg></span>
                    </button>
                    {activeYear === year && (
                        <div className="border-t border-gray-200 dark:border-gray-700">
                            {monthOrder.map((nome, i) => {
                                const monthKey = `${(i + 1).toString().padStart(2, '0')}${year}`;
                                const monthData = groupedData[year]?.[monthKey];
                                return (
                                    <div key={monthKey} className="border-b border-gray-200 dark:border-gray-700 last:border-b-0">
                                        <button onClick={() => monthData && setActiveMonthKey(activeMonthKey === monthKey ? null : monthKey)} className={`w-full text-left p-3 px-6 flex justify-between items-center ${monthData ? 'hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer' : 'opacity-50 cursor-default'}`}>
                                            <span className="text-gray-700 dark:text-gray-200">{nome.charAt(0).toUpperCase() + nome.slice(1)}</span>
                                            {monthData && <span className={`text-gray-400 transform transition-transform duration-200 ${activeMonthKey === monthKey ? 'rotate-180' : ''}`}><svg className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd" /></svg></span>}
                                        </button>
                                        {activeMonthKey === monthKey && (
                                            <div className="pl-8 pr-4 py-3 bg-gray-100 dark:bg-gray-900/50">
                                                {monthData?.files.length > 0 ? (
                                                    <ul className="space-y-2">{monthData.files.map(file => (
                                                        <li key={file.id} className="flex items-center space-x-3 p-1.5 text-gray-700 dark:text-gray-300">
                                                ...
                                                ) : <p className="text-sm text-gray-500 dark:text-gray-400 italic px-4">Nenhum arquivo para este mês.</p>}
                                            </div>
                                        )}
                                    </div>
                                )
...
const PastaManager = () => {
    ...
    const onDrop = useCallback((acceptedFiles, pastaTipo) => {
        ...
        const uploadPromises = acceptedFiles.map(file => {
            ...
            if (['xml', 'departamento_pessoal', 'simples_nacional'].includes(pastaTipo)) {
                formData.append('mes', (new Date().getMonth() + 1).toString().padStart(2, '0'));
        ...
        Promise.all(uploadPromises).then(() => { fetchData(); }).catch(err => {
            const errorMsg = err.response?.data?.detail || err.message || 'Erro no upload.';
            setError(errorMsg);
        }).finally(() => setUploading(false));
    }, [empresaNome, empresaCnpj, fetchData]);

    const { getRootProps, getInputProps, isDragActive } = useDropzone({
        onDrop: acceptedFiles => { if (selectedPasta) onDrop(acceptedFiles, selectedPasta.tipo) },
        noKeyboard: true, // Mantém noClick como false por padrão, permitindo o clique
    });
    ...
    const handlePastaClick = (pasta) => { setSelectedPasta(pasta); setSelectedFiles([]); setError(null); };
    ...
    const handleRefreshSelectedPasta = async () => {
        ...
        try {
            ...
            const errorMsg = err.response?.data?.error || `Falha ao sincronizar a pasta.`;
            setError(errorMsg);
        } finally {
    ...
    const handleWhatsAppClick = () => {
        ...
        axiosInstance.post(`/api/enviar-documentos-whatsapp/`, { empresa_id: empresaId, file_ids: selectedFiles, tipo_pasta: selectedPasta.tipo })
            .then(res => {
                let message = `Relatório do Envio por WhatsApp:\n`;
                if (res.data.successful_sends?.length > 0) {
                    message += `\nSucessos: ${res.data.successful_sends.map(s => s.filename).join(', ')}\n`;
                }
                if (res.data.failed_sends?.length > 0) {
    ...
    return (
        <div className="p-6 md:p-8">
            ...
            <div className="mb-8"><h2 className="text-lg font-semibold text-gray-700 dark:text-gray-300 mb-3">Pastas</h2><div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
                {pastaTypes.map(tipo => (
                    <motion.button key={tipo} onClick={() => handlePastaClick({ tipo, id: tipo })} whileHover={{ y: -5 }} className={`p-4 rounded-xl text-left transition-all duration-200 border-2 ${selectedPasta?.id === tipo ? 'bg-indigo-50 dark:bg-indigo-900/50 border-indigo-500 shadow-lg' : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 hover:border-indigo-400 dark:hover:border-indigo-500 hover:shadow-md'}`}>
                        <FolderIcon className={`h-8 w-8 mb-2 ${selectedPasta?.id === tipo ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-500 dark:text-gray-400'}`} />
                        <span className="font-semibold text-gray-800 dark:text-gray-100">{pastaConfig[tipo].label}</span>
                    </motion.button>
                ))}
            </div></div>
                ...
                <motion.div key={selectedPasta.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }}>
                    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 mt-8">
                        <div className="p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
                            ...
                            <button onClick={handleRefreshSelectedPasta} disabled={isRefreshingPasta || loading || uploading} className="p-2 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full transition-colors" title="Sincronizar Pasta">
                        ...
                        <div className="p-4 sm:p-6" {...getRootProps()}>
                            <input {...getInputProps()}/>
                            <div className={`border-2 border-dashed rounded-lg p-8 text-center cursor-pointer transition-all ${isDragActive ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/40' : 'border-gray-300 dark:border-gray-600 hover:border-indigo-400 dark:hover:border-indigo-500'}`}>
                                <div className="flex flex-col items-center text-gray-500 dark:text-gray-400">
                                    <ArrowUpOnSquareIcon className="mx-auto h-10 w-10 mb-2"/>
                                    <p>{uploading ? 'Enviando...' : (isDragActive ? 'Solte os arquivos aqui...' : 'Arraste arquivos ou clique para selecionar')}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-2 sm:p-4">
                            {loading ? <p className="text-center py-10 text-gray-500 dark:text-gray-400">Carregando arquivos...</p> : 
                            <>
                                {(selectedPasta.tipo === 'xml' || selectedPasta.tipo === 'departamento_pessoal' || selectedPasta.tipo === 'simples_nacional') ? (
                                    ...
                                    <ul className="space-y-2">{(arquivos[selectedPasta.tipo] || []).map(file => (
                                ...
                                {(!arquivos[selectedPasta.tipo] || arquivos[selectedPasta.tipo].length === 0) && <p className="text-center py-10 text-gray-500 dark:text-gray-400">Nenhum arquivo nesta pasta.</p>}
                            </>
                            }
                        </div>
                    </div>
