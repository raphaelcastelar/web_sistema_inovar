...
// Função auxiliar para gerar avatar (pode ser movida para um arquivo de utils se preferir)
...
const EmpresaList = () => {
    ...
    if (window.confirm('Tem certeza que deseja excluir esta empresa? Esta ação apaga também a pasta da empresa no servidor.')) {
      axiosInstance.delete(`/api/empresas/${id}/`)
        ...
        .catch(error => {
            ...
            alert('Falha ao excluir a empresa. Verifique o console para mais detalhes.');
        });
    }
  };
  ...
  const filteredEmpresas = useMemo(() => {
    ...
    if (!lowercasedSearch) {
      return empresas; // Se a busca for vazia, retorna todas as empresas
    }

    // Limpa a busca para procurar apenas por dígitos no CNPJ
      ...
      {filteredEmpresas.length === 0 ? (
        <div className="text-center py-16 px-4 bg-white dark:bg-gray-800 rounded-lg shadow-md">
            ...
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                {search ? 'Tente refinar sua busca ou ' : 'Comece cadastrando uma nova empresa.'}
                {!search && <Link to="/empresas/cadastrar" className="font-medium text-indigo-600 dark:text-indigo-400 hover:underline">cadastre a primeira</Link>}
            </p>
        </div>
      ) : (
        <motion.div 
