...
const ConsultarExtratoPage = () => {
    ...
    const yearOptions = useMemo(() => {
        ...
        const years = [];
        for (let i = -4; i <= 2; i++) { // Range de anos: 4 anos passados, atual, 2 futuros
    ...
    const handleConsultarExtrato = async () => {
        ...
        if (!empresaSelecionada) {
            setError("Erro: Empresa selecionada não encontrada na lista.");
            setLoading(false);
            return;
        }
        ...
        try {
            ...
            const errorDetail = err.response?.data?.error || "Ocorreu um erro ao consultar o extrato. Verifique se existe uma declaração para este período.";
    ...
    const handleDownloadPdf = async () => {
        if (!extratoData || !selectedEmpresaId) {
            setError("Dados do extrato não estão disponíveis para baixar o PDF.");
            return;
        }
        ...
        if (!numero_das) {
            setError("Não foi possível encontrar o Número do Documento para baixar o PDF do extrato.");
            return;
        }
        ...
        <div className="p-6 md:p-10 bg-gray-900 min-h-screen text-gray-100">
            ...
            <div className="max-w-xl mx-auto bg-gray-800 p-8 rounded-xl shadow-lg space-y-6">
                <div>
                    ...
                    <select id="empresa-select" value={selectedEmpresaId} onChange={(e) => setSelectedEmpresaId(e.target.value)} className="w-full p-3 bg-gray-700 text-white rounded-md mt-1 focus:ring-indigo-500 focus:border-indigo-500">
                ...
                <div>
                    ...
                    <div className="flex items-center gap-4 mt-1">
                        <select id="month-select" value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)} className="w-full p-3 bg-gray-700 text-white rounded-md focus:ring-indigo-500 focus:border-indigo-500">
                            {monthOptions.map(opt => (<option key={opt.value} value={opt.value}>{opt.label}</option>))}
                        </select>
                        <select id="year-select" value={selectedYear} onChange={(e) => setSelectedYear(e.target.value)} className="w-full p-3 bg-gray-700 text-white rounded-md focus:ring-indigo-500 focus:border-indigo-500">
                            {yearOptions.map(year => (<option key={year} value={year}>{year}</option>))}
                        </select>
                    </div>
                </div>
                <div className="pt-4">
                    <button onClick={handleConsultarExtrato} disabled={loading || !selectedEmpresaId} className="w-full flex items-center justify-center gap-2 px-6 py-3 bg-indigo-600 text-white rounded-md hover:bg-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed">
                        {loading ? 'Consultando...' : <><MagnifyingGlassIcon className="h-5 w-5"/> Consultar Extrato</>}
                    </button>
                </div>
            </div>

            {/* Container para Erros ou Resultados */}
            <div className="mt-6">
                {/* Exibe mensagem de erro geral */}
                {!loading && error && (
                    <div className="max-w-4xl mx-auto p-4 bg-red-900/30 border border-red-700 rounded-lg text-red-300 flex items-center justify-center gap-3">
                        <InformationCircleIcon className="h-6 w-6"/>
                        <span>{error}</span>
                    </div>
                )}
                
                {/* Exibe o resultado da consulta (agora diretamente aqui) */}
                {!loading && extratoData && (() => {
                    // Cenário: Declaração sem valor a pagar
                    if (extratoData.tipo === 'declaracao_sem_valor') {
                        const { periodoApuracao, mensagem } = extratoData;
                        return (
                    ...
                    // Cenário: Extrato detalhado com valor
                    if (extratoData.tipo === 'extrato_detalhado') {
                        const dasDetails = extratoData.declaracoes?.[0]?.das?.[0]?.detalhamentoDas;
                        if (!dasDetails) return null; // Segurança extra
                            ...
                            <div className="max-w-4xl mx-auto mt-10 bg-gray-800 p-8 rounded-xl shadow-lg animate-fade-in">
                                <div className="flex justify-between items-center border-b border-gray-700 pb-4 mb-6">
                                    <h2 className="text-2xl font-bold text-indigo-400">
                                        Extrato para {dasDetails.periodoApuracao ? `${dasDetails.periodoApuracao.substring(4, 6)}/${dasDetails.periodoApuracao.substring(0, 4)}` : ''}
                                    </h2>
                                    <button onClick={handleDownloadPdf} disabled={isDownloadingPdf} className="flex items-center gap-2 px-4 py-2 bg-teal-600 text-white rounded-md hover:bg-teal-500 disabled:opacity-50">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-6 text-center">
                                    <div className="bg-gray-700 p-4 rounded-lg"><p className="text-sm text-gray-400">Nº Documento</p><p className="text-xl font-semibold text-white break-all">{dasDetails.numeroDocumento || 'N/A'}</p></div>
                    ...
                    // Se o 'tipo' não for nenhum dos esperados, não renderiza nada
                    return null;
                })()}
            </div>
        </div>
