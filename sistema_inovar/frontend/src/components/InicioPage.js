...
const InicioPage = () => {
    // 1. A DECLARAÇÃO DO ESTADO ACONTECE AQUI
    // A função `setData` é criada nesta linha.
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            try {
                ...
                // 2. A CHAMADA ACONTECE AQUI, DENTRO DO MESMO ESCOPO
                // Como ambas estão dentro de `InicioPage`, isso funciona.
                setData(response.data);

            } catch (err) {
                const errorMessage = err.response?.data?.error || "Não foi possível carregar os dados do dashboard.";
                setError(errorMessage);
                console.error("Erro ao carregar dados do dashboard:", err.response || err);
            } finally {
                setLoading(false);
            }
        };
    ...
    const chartConfig = {
        ...
        options: {
            ...
            plugins: { legend: { position: 'bottom', labels: { color: document.documentElement.classList.contains('dark') ? 'white' : '#374151' } } },
            cutout: '70%',
        }
    };
    ...
    return (
        <motion.div initial="hidden" animate="visible" variants={containerVariants} className="p-6 md:p-8">
            <motion.h1 variants={itemVariants} className="text-3xl font-bold text-gray-800 dark:text-indigo-300 mb-8">
                Dashboard
            </motion.h1>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                <StatCard icon={UsersIcon} title="Total de Clientes" value={data.kpis.total_clientes} color="bg-blue-500"/>
                <StatCard icon={ClockIcon} title="Tarefas Pendentes (Total)" value={data.kpis.tarefas_pendentes} color="bg-yellow-500"/>
                <StatCard icon={ExclamationTriangleIcon} title="Vencendo em 7 dias" value={data.kpis.vencendo_em_7_dias} color="bg-red-500"/>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <motion.div variants={itemVariants} className="lg:col-span-2 bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
                    ...
                    <div className="space-y-4">
                        {data.proximas_tarefas.length > 0 ? data.proximas_tarefas.map(tarefa => (
                            <motion.div key={tarefa.id} variants={itemVariants} className="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg flex items-center justify-between">
                                <div>
                ...
                <div className="space-y-8">
                    ...
                    <motion.div variants={itemVariants} className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
                         ...
                         <div className="space-y-3">
                            <Link to="/gerar-das" className="w-full flex justify-between items-center p-3 text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors">
                                <span>Gerar Guia DAS</span><ArrowRightIcon className="h-5 w-5 text-gray-400"/>
                            </Link>
                             <Link to="/consultar-extrato" className="w-full flex justify-between items-center p-3 text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg transition-colors">
                                <span>Consultar Extrato</span><ArrowRightIcon className="h-5 w-5 text-gray-400"/>
                            </Link>
                         </div>
                    </motion.div>
                </div>
