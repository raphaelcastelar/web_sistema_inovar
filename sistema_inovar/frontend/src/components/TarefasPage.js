...
export default function TarefasPage() {
  ...
  return <div className="tasks-page">
    <header className="tasks-hero"><div><span><CheckCircleIcon /></span><div><p>{admin ? 'Visão da equipe' : 'Sua rotina'}</p><h1>Tarefas</h1><small>Organize tarefas e compromissos em um só lugar.</small></div></div><button onClick={() => setModal({ initial: { tipo: 'tarefa', dataPlanejada: today } })}><PlusIcon /> Nova atividade</button></header>
    ...
    {details && <Details activity={details} context={context} canEdit={canUpdate(details, context.usuario)} onClose={() => setDetails(null)} onEdit={() => openEdit(details)} />}
  </div>;
}
